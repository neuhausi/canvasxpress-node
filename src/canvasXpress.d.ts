// Type definitions for CanvasXpress (CommonJS — require("canvasxpress") = { init, cxplot, aes })
...
/**
 * The `canvasxpress` npm module (CommonJS / `require`): a namespace of
 * { init, cxplot, aes } where `init` is the CanvasXpress constructor.
 * ESM consumers (`import CanvasXpress from "canvasxpress"`) get the
 * constructor directly — see canvasxpress.d.mts.
 */
declare namespace CanvasXpress {
  ...
  export interface CXConfig {
     ...
     * @default 0
     */
    forecastHorizon?: number | boolean;
    /**
     * Level of the forecast prediction interval (e.g. 0.8 or 0.95)
     * @graphTypes Scatter2D
     * @default 0.95
     */
    forecastLevel?: number | boolean;
    /**
     * Line type for the projected forecast line
     * @graphTypes Scatter2D
     * @default "dashed"
     */
    forecastLineType?: "dashed" | "solid" | "dotted" | false | (string & {});
    /**
     * Forecast method. ses : simple exponential smoothing (level), holt : level and trend, hw :
     * Holt-Winters (level, trend and additive seasonality). auto uses hw when
     * forecastSeasonLength is greater than 1, holt otherwise. Short series fall back hw to holt
     * to ses
     * @graphTypes Scatter2D
     * @default "auto"
     */
    forecastMethod?: "auto" | "ses" | "holt" | "hw" | false | (string & {});
    /**
     * Number of observations per season (e.g. 12 for monthly data with a yearly cycle, 7 for
     * daily data with a weekly cycle). 0 or 1 disables seasonality
     * @graphTypes Scatter2D
     * @default 0
     */
    forecastSeasonLength?: number | boolean;
    /**
    ...
    /**
     * Name of a variable annotation, or set to true, to display a forecast (projected line and
     * prediction interval) in Scatter plots. The x values must be regularly spaced (numbers or
     * dates)
     * @graphTypes Scatter2D
     * @default false
     */
    showForecast?: string | boolean;
    /**
     * Flag to show/hide the forecast prediction interval
     * @graphTypes Scatter2D
     * @default true
     */
    showForecastInterval?: boolean;
    /**
    ...
    /**
     * Eviction policy applied by pushData() once the sample count exceeds streamWindow.
     * Currently only drop-oldest (remove the oldest samples first) is supported
     * @default "drop-oldest"
     */
    streamEvict?: "drop-oldest" | false | (string & {});
    /**
     * Maximum number of samples to retain when live-streaming via pushData(). When set, each
     * pushData() call appends the incoming samples and evicts the oldest ones beyond this window
     * so the matrix stays bounded for real-time updates. Null keeps every sample (no eviction)
     * @default null
     */
    streamWindow?: number | boolean;
    /**
  ...
  export type CXKeysFitLines =
    ...
    | "fitLineType"
    | "fitLineWidth";
  /** Config keys in the "Forecast" category. */
  export type CXKeysForecast =
    | "forecastHorizon"
    | "forecastLevel"
    | "forecastLineType"
    | "forecastMethod"
  ...
  export interface CXGraphTypeKeys {
    ...
    "Density": CXKeysAnimationsandVisualizationTransitions | CXKeysAspectRatioSpaceandWidth | CXKeysAxis | CXKeysBinPlots | CXKeysCitationsorReferences | CXKeysColors | CXKeysCombinationPlotStrips | CXKeysCombinationPlots | CXKeysConfidenceIntervals | CXKeysContextMenu | CXKeysContours | CXKeysDOE | CXKeysData | CXKeysDataContext | CXKeysDataCustomizer | CXKeysDataFaceting | CXKeysDataFilters | CXKeysDataPointAttributes | CXKeysDataRatioReference | CXKeysDataSelection | CXKeysDataSorting | CXKeysDataTable | CXKeysDataTimeSeries | CXKeysDataTransformation | CXKeysDataUIWidgets | CXKeysDebug | CXKeysDecorations | CXKeysDensityPlots | CXKeysErrorBars | CXKeysEvents | CXKeysFitLines | CXKeysForecast | CXKeysForegroundandBackground | CXKeysGeneral | CXKeysGeneralAttributes | CXKeysGradients | CXKeysGraphMetadata | CXKeysGreyOut | CXKeysHTMLWidgets | CXKeysHistograms | CXKeysImages | CXKeysKaplanMeierPlots | CXKeysLLM | CXKeysLines | CXKeysLoess | CXKeysManhattanPlots | CXKeysMarginalText | CXKeysMargins | CXKeysMissingData | CXKeysPlotDataType | CXKeysPlotMode | CXKeysPlotPerformance | CXKeysPlotView | CXKeysPlotandPanelAreas | CXKeysPrint | CXKeysQuantileRegressionPlots | CXKeysR | CXKeysRandom | CXKeysRegression | CXKeysRidgelinePlots | CXKeysSamples | CXKeysScatterPlotMatrix | CXKeysScatterPlots | CXKeysScrollbar | CXKeysShadows | CXKeysShapes | CXKeysSizes | CXKeysStreamPlots | CXKeysText | CXKeysTextPlacement | CXKeysTitlesandSubtitles | CXKeysTooltip | CXKeysTransparency | CXKeysVariables | CXKeysVisiumPlots | CXKeysWorkflowCharts | CXKeysXAxis | CXKeysYAxis | CXKeysZoomingPanningandResizing | CXKeyscxplot;
  ...
  /** The CanvasXpress constructor: `new CanvasXpress.init(target, data, config)`. */
  const init: CanvasXpressConstructor;
  /** ggplot-style plotting entry point. */
  const cxplot: CXPlotFn;
  /** Aesthetic-mapping helper used by the cxplot grammar. */
  const aes: CXAesFn;
}
